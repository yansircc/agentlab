...
func (s *replayState) checkpoint(record ledger.Record) error {
	...
	if s.exit != nil || s.started == nil || s.started.Adapter == nil || json.Unmarshal(record.Data, &value) != nil || !validRef(value.Checkpoint) || !validRef(value.PublicPrefix) || value.Intent.Validate() != nil || value.Intent.Kind != effect.Checkpoint {
