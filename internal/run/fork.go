...
func (value SessionForked) Validate() error {
	if value.Intent.Validate() != nil || value.Intent.Kind != effect.Fork {
...
func (o *Operation) RecordSessionForked(intent effect.Intent, spec SessionForkSpec) (SessionForked, error) {
	if intent.Validate() != nil || intent.RunID != o.runID || intent.Kind != effect.Fork || !validRef(spec.ExpectedCheckpoint) || len(spec.ChildSession) == 0 || len(spec.ObservedPrefix) == 0 || len(spec.AdapterIdentity) == 0 {
	...
	value := SessionForked{Intent: intent, ExpectedCheckpoint: spec.ExpectedCheckpoint, ParentSession: checkpoint.Session, ChildSession: child, ObservedPrefix: observed, AdapterIdentity: identity}
