...
func newSandbox(spec SandboxSpec) (Sandbox, error) {
	...
	return Sandbox{launcher: sandboxExecutable, profile: profile, workspace: workspace, runtimeRoot: runtimeRoot}, nil
}
...
func sandboxExecutables(values []string) ([]string, error) {
	...
	for _, value := range values {
		...
		if err != nil || statErr != nil || !info.Mode().IsRegular() || !profilePath(path) || seen[path] {
			return nil, errors.New("coder sandbox executable is invalid")
		}
		seen[path] = true
