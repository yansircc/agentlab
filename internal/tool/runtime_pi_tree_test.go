...
func TestPiRuntimeTreeIsBoundedPublicOnlyAndReturnsHostRef(t *testing.T) {
	...
	if !ok || page.Contract != runtimeTreePageContract || page.RuntimeRef != "worker-profile" || len(page.Entries) != 1 || page.NextAfter == nil || !page.Entries[0].PublicTextTruncated || page.Entries[0].StructurallyForkable == false {
...
func TestInspectRuntimeTreeAcceptsNoProviderRuntimeLocator(t *testing.T) {
	...
	result, err := Execute(Binding{Root: root, ExperimentID: "exp", Runtime: host}, value)
	if err != nil {
		t.Fatal(err)
	}
